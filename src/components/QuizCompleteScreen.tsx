...
import { CheckCircle2, RotateCcw, BarChart3, List, ChevronLeft, ChevronRight, Users, LayoutList, FileText, Save, Download, Trash2 } from 'lucide-react';
      ...
      <div className="mt-6 flex-1">
        ...
        {activeTab === 'overview' && config.type !== 'poll' && (
          <div className="grid grid-cols-1 md:grid-cols-3 gap-6">
            <div className="col-span-1 md:col-span-2 bg-white rounded-3xl p-8 border border-slate-200 shadow-sm flex items-center justify-center flex-col text-center">
              <h2 className="text-lg font-bold text-slate-500 uppercase tracking-widest mb-6">{config.type === 'poll' ? 'Overall Participation' : 'Class Accuracy'}</h2>
              <div className="relative w-48 h-48 flex items-center justify-center">
                <svg viewBox="0 0 100 100" className="w-full h-full -rotate-90">
                  ...
                  <circle
                    cx="50" cy="50" r="40" fill="none"
                    stroke="#10B981" strokeWidth="12"
                    strokeDasharray={`${(config.type === 'poll' ? (reportData.totalResponses / (questions.length * reportData.rosterSize)) : (reportData.overallAccuracy / 100)) * 251.2} 251.2`}
                    strokeLinecap="round"
                  />
                </svg>
                <div className="absolute inset-0 flex flex-col items-center justify-center">
                  <span className="text-4xl font-extrabold text-slate-800">{config.type === 'poll' ? Math.round((reportData.totalResponses / (questions.length * reportData.rosterSize)) * 100) : reportData.overallAccuracy}%</span>
                </div>
              </div>
              <p className="text-slate-600 mt-6 max-w-md font-medium">
                {config.type === 'poll' ? 'Thank you for participating!' : (reportData.overallAccuracy > 70 ? 'Great job! The class performed well overall.' : 'The class might need to review this topic.')}
              </p>
            </div>
            ...
            <div className="flex flex-col gap-6">
              <div className="bg-white p-6 rounded-3xl border border-slate-200 shadow-sm">
                <div className="flex items-center gap-3 text-amber-600 mb-2">
                  ...
                  <span className="font-bold uppercase tracking-wider text-sm">Questions</span>
                </div>
                <span className="text-4xl font-extrabold text-slate-800">{questions.length}</span>
              </div>
              <div className="bg-white p-6 rounded-3xl border border-slate-200 shadow-sm">
                <div className="flex items-center gap-3 text-amber-600 mb-2">
                  <Users className="w-5 h-5" />
                  <span className="font-bold uppercase tracking-wider text-sm">Participation</span>
                </div>
                <div className="flex items-baseline gap-2">
                  <span className="text-4xl font-extrabold text-slate-800">
                    {Math.round((reportData.totalResponses / (questions.length * reportData.rosterSize)) * 100)}%
                  </span>
              ...
              <div
                onClick={handleSaveClassReport}
                className="bg-gradient-to-r from-amber-500 to-purple-600 p-6 rounded-3xl border border-amber-400 shadow-sm flex flex-col justify-center items-center h-full text-center hover:shadow-md hover:-translate-y-1 transition-all cursor-pointer group"
              >
                <div className="w-12 h-12 bg-white/20 text-white rounded-full flex items-center justify-center mb-3 group-hover:scale-110 transition-transform">
                  <Save className="w-6 h-6" />
                </div>
                <h3 className="font-bold text-white">Save Class Report</h3>
                <p className="text-xs text-amber-100 mt-1">Save complete overview to Dashboard</p>
              </div>
            </div>
          </div>
        )}
            ...
            <div className="flex-1 space-y-6">
              ...
              <div className="bg-white border border-slate-200 p-6 rounded-3xl shadow-sm">
                <div className="flex items-center justify-between mb-6">
                  ...
                  <span className="text-xs font-bold text-slate-400">{activeStats.totalResponded} of {reportData.rosterSize} responded</span>
                </div>

                <div className="space-y-4">
                  {activeQuestion.options.map((opt, optIdx) => {
                    const count = activeStats.optionCounts[optIdx];
                    const percentage = reportData.rosterSize > 0 ? Math.round((count / reportData.rosterSize) * 100) : 0;
            ...
            <div className="w-full lg:w-80 shrink-0 bg-white border border-slate-200 rounded-3xl p-6 shadow-sm flex flex-col h-[600px]">
              <div className="mb-4">
                ...
                <div className="text-xl font-bold text-slate-800 mt-1">
                  {activeStats.totalResponded} / {reportData.rosterSize} <span className="text-sm font-medium text-slate-500">responded</span>
                </div>
              </div>

              <div className="flex-1 overflow-y-auto pr-2 space-y-2 pb-4 scrollbar-thin scrollbar-thumb-slate-200">
                {Array.from({ length: reportData.rosterSize }).map((_, i) => {
                  const roll = i + 1;
                  const responseIdx = activeStats.studentResponses[i];
                  ...
                  return (
                    <div key={roll} className="flex items-center justify-between py-2 border-b border-slate-50 last:border-0">
                      <div className="flex items-center gap-3">
                        ...
                        <span className="text-sm font-medium text-slate-700 truncate w-32" title={`${roll} ${findStudentByRoll(roll)?.name || '(Unregistered)'}`}>
                          {roll} {findStudentByRoll(roll)?.name || '(Unregistered)'}
                        </span>
                      </div>
          ...
          <div className="flex flex-col lg:flex-row gap-6">
            ...
            <div className="w-full lg:w-64 bg-white rounded-3xl border border-slate-200 shadow-sm p-4 flex flex-col h-[600px]">
              ...
              <div className="flex-1 overflow-y-auto space-y-1 pr-2 custom-scrollbar">
                {Array.from({ length: config.rollCount || 30 }, (_, i) => i + 1).map(roll => {
                  const sData = studentReportData[roll];
                  const totalAns = sData.correct + sData.incorrect;
                  return (
                    <button
                      key={roll}
                    ...
                    >
                      <span>{roll} {findStudentByRoll(roll)?.name || '(Unregistered)'}</span>
                      {totalAns > 0 ? (
